...
public class SortableList<T extends Comparable<? super T>> extends SinglyLinkedList<T> {
    ...
    /**
     * Calling list and otherList are both sorted, this method will merge the right list into calling list
     * while maintaining the sorted order. Uses O(1) extra space
     *
     * @param otherList : A sorted list
     */
    ...
    /**
     * Performs merge sort on the calling list, by recursively dividing and rearranging elements of the list.
     * Uses O(lg n) space for recursive calls, no extra space since it does not create any new entries and works by
     * rearranging pointers of the existing entries
     */
    void mergeSort() {
        if (this.head.next.next != null) { // more than one element in the list
            SortableList<T> right = new SortableList<>(); // only O(1) space for creating dummy entry
            ...
            this.mergeSort();
            right.mergeSort();
            // rearrange this list and right sorted list as one list, using this list
            this.merge(right);
        } else {
    ...
    private Entry<T> getMid() {
        ...
        Entry<T> c = p2.next;
        // Invariant: at the end of every iteration p1 moves to next element, p2 skip one element ahead
        // and c is next of p2
    ...
    /**
     * This method will sort  the list passed as a parameter using merge sort and log n space
     *
     * @param list : SortableList : List to be sorted
     */
